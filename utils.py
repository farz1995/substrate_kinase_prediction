...
def prepare_saving_dir(configs):
    ...
    # shutil.copy(configs, result_path)
...
def visualize_predictions(model, dataloader, device, num_sequences=5):
    ...
    for i in range(len(sequences)):
        ...
        # Highlight all 'Y's in green
        highlighted_seq = ''.join(
            f"{Fore.GREEN}{char}{Style.RESET_ALL}" if char == 'Y' else char
            for char in decoded_seq
        )

        pred_str = ' '.join('P' if p else '.' for p in pred)  # Highlight positive predictions
        ...
        print(f"\nSequence {i + 1}:")
        print(f"  Sequence:      {highlighted_seq}")
        ...
        print(
            ...
            f"Correctly Predicted Positives: {true_positives}, False Positives: {false_positives}"
        )